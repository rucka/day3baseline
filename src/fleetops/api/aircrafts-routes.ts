...
import { query } from "@core/data-access/db";
...
router.get("/:model", async (req, res) => {
  try {
    const result = await query(
      "SELECT * FROM fleetops.aircrafts WHERE model = $1",
      [req.params.model]
    );
...
router.post("/", async (req, res) => {
  try {
    const {
      ...
      cruise_speed_m,
      engine_type,
      noise_level_epndb_min,
      noise_level_epndb_max,
      __version,
    } = req.body;
    const result = await query(
...
router.put("/:model", async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      return res
        .status(409)
        .json({ message: "Aircraft model not found or version mismatch" });
    }
  } catch (error: any) {
    return res.status(500).json({ error: error.message });
  }

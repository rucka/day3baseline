...
import { pool, query } from "@core/data-access/db";
...
router.put("/:id", async (req, res) => {
  ...
  if (isNaN(expectedVersion)) {
    return res
      .status(400)
      .json({ message: "Invalid or missing expectedVersion query parameter" });
  }
  ...
  if (req.params.id != cabinLayout.layoutId) {
    return res
      .status(400)
      .json({ message: "Invalid layout id in request body" });
  }
...
async function isLayoutValid(layout: CabinLayout): Promise<boolean> {
  ...
  function getWidth(seatTypeId: string): number {
    return (
      seatTypes.find((st) => st.seat_type_id === seatTypeId)?.width_cm || 0
    );
  }

  function getPitch(seatTypeId: string): number {
    return (
      seatTypes.find((st) => st.seat_type_id === seatTypeId)?.pitch_cm || 0
    );
  }
  ...
  let totalLength = layout.rows.reduce(
    (a, b) => a + getPitch(b.seatType) + b.extraSpace,
    0
  );
